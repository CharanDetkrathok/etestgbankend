...
public class GetRegisterSelectReport extends HttpServlet { 
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (examdate.equals("0") && sec.equals("0")) {
            ...
            getRecieptDataList = getRepcieptTable.findRegisterBySection(sem, year, sec);
            if (getRecieptDataList == null) {
                response.sendRedirect("admin/faild.jsp");
            }
        } else {
            if (sec.equals("0")) {
                getRecieptDataList = getRepcieptTable.findRegisterByDate(sem, year, examdate);
                if (getRecieptDataList == null) {
                    response.sendRedirect("admin/faild.jsp");
                }
            } else {
                getRecieptDataList = getRepcieptTable.findRegisterByDateAndSection(sem, year, sec, examdate);
                if (getRecieptDataList == null) {
                    response.sendRedirect("admin/faild.jsp");
                }
            }
        ...
        RequestDispatcher rs = request.getRequestDispatcher("admin/ShowStdRegisAllFromSelect.jsp");
       ...
       /* PrintWriter out = response.getWriter();
        try {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GetRegisterSelectReport</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet GetRegisterSelectReport at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {
            out.close();
