...
public class ET_RECEIPT_TABLE {
    ...
    public List<ET_RECEIPT> findAllDateAllSection(String year, String secmester) {
        List<ET_RECEIPT> list = new ArrayList<ET_RECEIPT>();
        String sql = "SELECT DISTINCT A.FISCAL_YEAR,A.RECEIPT_YEAR,A.RECEIPT_SEMESTER,A.STD_CODE, A.TOTAL_AMOUNT,A.RECEIPT_STATUS,A.FACULTY_NO, A.REF_KEY,A.RECEIPT_PAY_STATUS,TO_CHAR(A.REGIS_DATE, 'dd/mm/yyyy hh24:mi:ss','NLS_CALENDAR=''THAI BUDDHA'' NLS_DATE_LANGUAGE=THAI')REGIS_DATE,D.NAME_THAI FROM ET_RECEIPT A,ET_REGIS_RU24 B,QR_PAYMENT_CONFIRM_TMB C,DBBACH00.VM_STUDENT_MOBILE D WHERE A.STD_CODE = B.STD_CODE AND A.RECEIPT_SEMESTER = B.SEMESTER AND A.RECEIPT_YEAR = B.YEAR AND A.REF_KEY = B.REF_KEY AND A.STD_CODE = C.STD_CODE(+) AND A.REF_KEY = C.QRID(+) AND TO_NUMBER(A.TOTAL_AMOUNT) = TO_NUMBER(C.AMOUNT(+)) AND B.STD_CODE = D.STD_CODE AND A.RECEIPT_YEAR = C.YEAR(+) AND A.RECEIPT_SEMESTER = C.SEMESTER(+) AND A.RECEIPT_SEMESTER = '"+secmester+"' AND A.RECEIPT_YEAR = '"+year+"' ORDER BY D.NAME_THAI ASC";
        List<Map<String, Object>> result = db.queryList(sql);
    ...
    public List<ET_RECEIPT> findReceiptDetail(String year, String secmester, String stdCode, String RefKey) {
        List<ET_RECEIPT> list = new ArrayList<ET_RECEIPT>();
        String sql = " SELECT DISTINCT A.YEAR,A.SEMESTER,A.STD_CODE,A.COURSE_NO,A.CREDIT,"
    ...
    public List<ET_RECEIPT> findReceiptAllDateExamAllSecoet(String sem, String year) {
        List<ET_RECEIPT> list = new ArrayList<ET_RECEIPT>();
        String sql = "SELECT DISTINCT A.FISCAL_YEAR,A.RECEIPT_YEAR,A.RECEIPT_SEMESTER,A.STD_CODE, A.TOTAL_AMOUNT,A.RECEIPT_STATUS,A.FACULTY_NO, A.REF_KEY,A.RECEIPT_PAY_STATUS,TO_CHAR(A.REGIS_DATE, 'dd/mm/yyyy hh24:mi:ss','NLS_CALENDAR=''THAI BUDDHA'' NLS_DATE_LANGUAGE=THAI')REGIS_DATE,D.NAME_THAI FROM ET_RECEIPT A,ET_REGIS_RU24 B,QR_PAYMENT_CONFIRM_TMB C,DBBACH00.VM_STUDENT_MOBILE D WHERE A.STD_CODE = B.STD_CODE AND A.RECEIPT_SEMESTER = B.SEMESTER AND A.RECEIPT_YEAR = B.YEAR AND A.REF_KEY = B.REF_KEY AND A.STD_CODE = C.STD_CODE(+) AND A.REF_KEY = C.QRID(+) AND TO_NUMBER(A.TOTAL_AMOUNT) = TO_NUMBER(C.AMOUNT(+)) AND B.STD_CODE = D.STD_CODE AND A.RECEIPT_YEAR = C.YEAR(+) AND A.RECEIPT_SEMESTER = C.SEMESTER(+) AND A.RECEIPT_SEMESTER = '"+sem+"' AND A.RECEIPT_YEAR = '"+year+"' AND A.RECEIPT_PAY_STATUS = '1' ORDER BY D.NAME_THAI ASC";
        List<Map<String, Object>> result = db.queryList(sql);
    ...
    public List<ET_RECEIPT> findRegisterAll(String sem, String year) {
        List<ET_RECEIPT> list = new ArrayList<ET_RECEIPT>();
        String sql = "SELECT DISTINCT A.FISCAL_YEAR,A.RECEIPT_YEAR,A.RECEIPT_SEMESTER,A.STD_CODE, A.TOTAL_AMOUNT,A.RECEIPT_STATUS,A.FACULTY_NO, A.REF_KEY,A.RECEIPT_PAY_STATUS,TO_CHAR(A.REGIS_DATE, 'dd/mm/yyyy hh24:mi:ss','NLS_CALENDAR=''THAI BUDDHA'' NLS_DATE_LANGUAGE=THAI')REGIS_DATE,D.NAME_THAI FROM ET_RECEIPT A,ET_REGIS_RU24 B,QR_PAYMENT_CONFIRM_TMB C,DBBACH00.VM_STUDENT_MOBILE D WHERE A.STD_CODE = B.STD_CODE AND A.RECEIPT_SEMESTER = B.SEMESTER AND A.RECEIPT_YEAR = B.YEAR AND A.REF_KEY = B.REF_KEY AND A.STD_CODE = C.STD_CODE(+) AND A.REF_KEY = C.QRID(+) AND TO_NUMBER(A.TOTAL_AMOUNT) = TO_NUMBER(C.AMOUNT(+)) AND B.STD_CODE = D.STD_CODE AND A.RECEIPT_YEAR = C.YEAR(+) AND A.RECEIPT_SEMESTER = C.SEMESTER(+) AND A.RECEIPT_SEMESTER = '"+sem+"' AND A.RECEIPT_YEAR = '"+year+"' ORDER BY D.NAME_THAI ASC";
        List<Map<String, Object>> result = db.queryList(sql);
