...
public class GENERATE_ET_EXAM_SEAT_TABLE {
    ...
    public List<GENERATE_ET_EXAM_SEAT> findExamDateAndSectionForStudents(String year, String semester, String examDate, String section) {
        List<GENERATE_ET_EXAM_SEAT> list = new ArrayList<GENERATE_ET_EXAM_SEAT>();
        String sql = " SELECT DISTINCT(A.REF_KEY), TO_CHAR(A.EXAM_DATE, 'MM/DD/YYYY')EXAM_DATE, A.STD_CODE,A.COURSE_NO,A.CREDIT, A.SECTION_NO,B.RECEIPT_PAY_STATUS"
                ...
                + " AND B.RECEIPT_PAY_STATUS = '1' AND A.STD_CODE = B.STD_CODE AND A.REF_KEY = B.REF_KEY  AND B.REF_KEY = C.QRID AND TO_NUMBER(B.TOTAL_AMOUNT) = TO_NUMBER(C.AMOUNT)"
                + " ORDER BY EXAM_DATE,A.STD_CODE";
        List<Map<String, Object>> result = db.queryList(sql);
