...
public class GENERATE_ET_EXAM_SEAT_TABLE {
    ...
    public List<GENERATE_ET_EXAM_SEAT> findExamDateAndSectionForStudents(String year, String semester, String examDate, String section) {
        List<GENERATE_ET_EXAM_SEAT> list = new ArrayList<GENERATE_ET_EXAM_SEAT>();
        String sql = "SELECT DISTINCT TO_CHAR(B.RECEIPT_DATE, 'MM/DD/YYYY hh:mm:ss') INSERT_DATE, TO_CHAR(A.EXAM_DATE, 'MM/DD/YYYY')EXAM_DATE,B.COUNTER_NO, A.STD_CODE,A.COURSE_NO,A.CREDIT, A.SECTION_NO,B.RECEIPT_PAY_STATUS FROM ET_REGIS_RU24 A,ET_RECEIPT B,QR_PAYMENT_CONFIRM_TMB C WHERE A.YEAR = '" + year + "' AND A.SEMESTER = '" + semester + "' AND A.EXAM_DATE = TO_DATE('" + examDate + "', 'MM/DD/YYYY') AND A.SECTION_NO = '" + section + "' AND B.RECEIPT_PAY_STATUS = '1' AND A.STD_CODE = B.STD_CODE AND A.REF_KEY = B.REF_KEY AND A.STD_CODE = C.STD_CODE AND A.REF_KEY = C.QRID AND C.YEAR = '" + year + "' AND C.SEMESTER = '" + semester + "' AND B.REF_KEY = C.QRID AND B.RECEIPT_SEMESTER = '" + semester + "' AND B.RECEIPT_YEAR = '" + year + "' AND TO_NUMBER(B.TOTAL_AMOUNT) = TO_NUMBER(C.AMOUNT) AND NOT EXISTS (SELECT L.STD_CODE,L.COURSE_NO FROM ET_ROW_SEAT_ORDER L WHERE A.STD_CODE = L.STD_CODE AND A.COURSE_NO = L.COURSE_NO AND A.YEAR = L.YEAR AND A.SEMESTER = L.SEMESTER AND A.SECTION_NO = L.SECTION_NO AND L.STD_CODE = B.STD_CODE AND L.YEAR = B.RECEIPT_YEAR AND L.SEMESTER = B.RECEIPT_SEMESTER AND L.STD_CODE = C.STD_CODE AND L.YEAR = C.YEAR AND L.SEMESTER = C.SEMESTER) ORDER BY TO_CHAR(B.RECEIPT_DATE,'MM/DD/YYYY hh:mm:ss')";
        List<Map<String, Object>> result = db.queryList(sql);
